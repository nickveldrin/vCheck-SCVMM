...
$Display = "Table"
$Author = "Jan Egil Ring - modified by Nick M"
$PluginVersion = 1.1
			...
			If ((Get-SCVMHost -ComputerName $hostName).OverallState -eq "OK"){
				...
				foreach($guestName in $guests) {
					Try {
						...
						$kvpHash = @{}
						if($guestkvp.GuestIntrinsicExchangeItems){
							$xmlContents = ([xml]("<xml>"+$guestkvp.GuestIntrinsicExchangeItems+"</xml>"))
